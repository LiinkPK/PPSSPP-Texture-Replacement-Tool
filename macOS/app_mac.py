...
def refresh_sidebar():
    ...
    for i, game in enumerate(history_data):
        ...
        if IS_MAC:
            item_frame.bind("<Button-2>", on_right_click)
            name_lbl.bind("<Button-2>", on_right_click)
...
def make_vertical_label(text, color, fontsize=18):
    try:
        if IS_MAC:
            font_path = "/Library/Fonts/TrajanPro-Regular.ttf"
            if not Path(font_path).exists():
                font_path = str(Path.home() / "Library/Fonts/TrajanPro-Regular.ttf")
        else:
            font_path = "C:/Users/javi_/AppData/Local/Microsoft/Windows/Fonts/TrajanPro-Regular.ttf"
...
def load_preview(path, canvas, label_info):
    ...
    if canvas == old_canvas:
        ...
        if IS_MAC:
            old_canvas.bind("<Button-2>", lambda e: canvas_context_menu(e, "old"))
    else:
        new_img_ref = photo
        new_img_full = path
        ...
        if IS_MAC:
            new_canvas.bind("<Button-2>", lambda e: canvas_context_menu(e, "new"))
...
if IS_MAC:
    sidebar_list_frame.bind("<Button-2>", sidebar_right_click)
    sidebar_title.bind("<Button-2>", sidebar_right_click)

sidebar_btn_frame = tk.Frame(root, bg=DARK["sidebar_bg"])
sidebar_btn_frame.grid(row=4, column=0, sticky="ew")

add_game_btn = make_rounded_button(sidebar_btn_frame, "+ Add Game Folder", add_game_to_history, font=FONT_SMALL, width=212, height=30, bg=NAVY, fg="#ffffff", hover_bg=NAVY_DARK, parent_bg=DARK["sidebar_bg"])
...
def make_link(parent, text, url, col):
    btn = make_rounded_button(parent, text, lambda: webbrowser.open(url), font=FONT_SMALL, width=80, height=28, bg=current_theme["footer_bg"], fg=NAVY, hover_bg=current_theme["footer_bg"], parent_bg=current_theme["footer_bg"])
